...
function FidelityFunctionCurve=DynamicRangeTest(TestSetting,EncodingMatrix,Wavelength_index,decode_method,draw_flag)
...
if draw_flag==1
    %% fidelity curve
    figure
    plot(DR_dB,FidelityFunctionCurve)    
    xlabel('DynamicRangeGain/dB')
    ylabel('Fidelity')
    title(sprintf('Fidelity function of DynamicRangeGain \n *PeakPosition= %d nm,Resolution= %d nm',PeakPosition,Resolution))
    exportgraphics(gca, 'Fidelity function of DynamicRangeGain.tif', 'Resolution', 300);
    %% slice check 1
    figure
    ...
    exportgraphics(gca, 'Fidelity function of DynamicRangeGain BEST.tif', 'Resolution', 300);
    %% slice check 2
    figure
    ...
    legend('GT','Reconstructed')
    exportgraphics(gca, 'Fidelity function of DynamicRangeGain WORST.tif', 'Resolution', 300);
end
